...
# Get Amplify App ID from amplify_outputs.json if available
if [ -f "amplify_outputs.json" ]; then
    APP_ID=$(grep -o '"app_id":"[^"]*"' amplify_outputs.json | cut -d'"' -f4)
    if [ -z "$APP_ID" ]; then
        echo "⚠️  Could not find app_id in amplify_outputs.json"
        echo "Please check AWS Console manually"
        exit 1
    fi
else
    echo "⚠️  amplify_outputs.json not found"
    echo "Please provide your Amplify App ID manually"
    exit 1
fi
