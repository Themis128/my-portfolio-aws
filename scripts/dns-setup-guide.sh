...
if [[ -n "$ROOT_ALIAS" && "$ROOT_ALIAS" == *"dtxzcxrjfr622.cloudfront.net"* ]]; then
    ...
    echo "      Alias Target: dtxzcxrjfr622.cloudfront.net"
...
if [[ -n "$WWW_EXISTS" && "$WWW_EXISTS" == *"dtxzcxrjfr622.cloudfront.net"* ]]; then
    ...
    echo "      Type: CNAME"
    echo "      Name: www"
    echo "      Value: dtxzcxrjfr622.cloudfront.net"
fi
...
if [[ -z "$ROOT_ALIAS" || "$ROOT_ALIAS" != *"dtxzcxrjfr622.cloudfront.net"* ]]; then
    echo "2. 📝 Add the Root Domain A record (alias)"
    MISSING_RECORDS=true
fi

if [[ -z "$WWW_EXISTS" || "$WWW_EXISTS" != *"dtxzcxrjfr622.cloudfront.net"* ]]; then
