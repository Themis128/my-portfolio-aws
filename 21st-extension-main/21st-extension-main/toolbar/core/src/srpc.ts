...
export async function discoverVSCodeWindows(
    ...
    try {
      ...
      if (response.ok && (await response.text()) === PING_RESPONSE) {
        ...
        try {
          ...
          console.debug(`Failed to get session info from port ${port}:`, error);
        }
      } else {
      ...
      // Stop searching after N consecutive connection errors to avoid noisy output
      if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
        console.debug(
          `Discovery stopped after ${consecutiveErrors} consecutive connection errors (expected during normal operation).`,
        );
        break;
      }
  ...
  if (windows.length === 0) {
    // Use debug logging for no-windows case so it doesn't spam developer consoles.
    console.debug(
      `No IDE windows found. Start an IDE with the stagewise extension if you want integration.`,
    );
  }
